...
namespace WasteIntoCity.Core.Models
{
    public class ScoreSettingsType
    {
        ...
        public static ScoreSettingsType Create(ScoreSettingsEnum id, MeanText name, int value)
        {
            ...
            {
                throw new InvalidLengthException(12, $"{nameof(name)} of {nameof(ScoreSettingsType)}", NAME_LENGTH_MIN, NAME_LENGTH_MAX);
            }

            if (value is < VALUE_MIN or > VALUE_MAX)
            {
                throw new ValueOutOfRangeException<int>($"{nameof(value)} of {nameof(ScoreSettingsType)}", VALUE_MIN, VALUE_MAX, 41);
            }
